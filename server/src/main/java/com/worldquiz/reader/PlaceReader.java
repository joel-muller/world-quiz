...
public class PlaceReader {
    ...
    public List<Place> read() {
        Map<String, Place.PlaceBuilder> builders = this.readMain(this.readCsv("main.csv"));
        updatePlaceBuilder(
                this.readCsv("country_info.csv"), builders, Place.PlaceBuilder::placeInfo);
        updatePlaceBuilder(this.readCsv("capital.csv"), builders, Place.PlaceBuilder::capital);
        updatePlaceBuilder(
                this.readCsv("capital_info.csv"), builders, Place.PlaceBuilder::capitalInfo);
        updatePlaceBuilder(
                this.readCsv("flag_similarity.csv"), builders, Place.PlaceBuilder::flagInfo);
            ...
            Place.PlaceBuilder builder =
                    Place.builder()
                            ...
                            .tags(
                                    getTags(
                                            Arrays.toString(
                                                    Arrays.copyOfRange(line, 4, line.length))));
    ...
    private void updatePlaceBuilder(
        ...
        for (String[] line : lines) {
            Place.PlaceBuilder builder = builders.get(line[0]);
            if (builder != null) {
