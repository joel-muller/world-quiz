...
public class CardManager {
    ...
    public List<Card> getCards(int numberOfCards, Category category, Set<Tag> tags) {
        ...
        // TODO: Shuffle the places
        return this.places.stream()
                .filter(p -> isCategory(p, category) && hasAtLeastOneTag(p, tags))
                .map(p -> getCard(p, category))
                .limit(numberOfCards)
                .toList();
    }
        ...
        return switch (category) {
            case MAP_NAME ->
                    new Card(
                            ...
                            place.placeInfo() + " " + place.capitalInfo());
            case FLAG_NAME ->
                    new Card(
                            place.id(),
                            category,
                            ...
                            "",
                            place.nameWithCapital(),
                            place.placeInfo() + " " + place.capitalInfo() + " " + place.flagInfo());
            case CAPITAL_NAME ->
                    new Card(
                            place.id(),
                            category,
